...
<body>
    ...
    <div class="container container-content">
        ...
        @endif
        {{-- <div class="container">
            <form action="{{ url('pay/' . $den->id) }}" method="POST" class="row justify  -content-center">
                @csrf
                <div class="col-md-6">
                    <input type="number" class="form-control" name="pay" placeholder="Amount payed " required>
                </div>
                <div class="col-md-6">
        ...
        <div class="data-table">
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <h3 style="color: red">Total= {{ $totalDebt }}/=</h3>
                    <hr>
                    {{-- <h4>Number of stock sold = <b style="color: red">{{ $countt }}</b></h4> --}}
                    <tbody>
                        @foreach ($debt as $debit)
                            <tr>
                                <td>{{ $debit->id }}</td>
                                <td>{{ $debit->name }}</td>
                                <td>{{ $debit->things }}</td>
                        ...
                        <td>{{$debt->amount}}</td> --}}
                                ...
                                <td>
                                    <a href="{{ url('delete_debt/' . $debit->id) }}">
